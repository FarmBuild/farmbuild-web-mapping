angular.module('farmbuild.webmapping.examples', ['farmbuild.webmapping'])
	...
	function ($scope, $log, $location, $rootScope, webmapping) {
		...
		function mapOnFeatureSelect(event, data) {
			...
			$scope.farmData.selectedPaddockName = feature.getProperties().name;
			$scope.farmData.selectedPaddockArea = feature.getGeometry().getArea() * 0.0001;
			$log.info('Paddock selected: ' + $scope.farmData.selectedPaddockName);
			$scope.$apply();
		}

		function mapOnFeatureDeselect(event) {
			console.log(event);
			var selectedLayer = layerSelectionElement.value;
				if ((selectedLayer !== 'paddocks')) {
				$scope.farmData.selectedPaddockName = '';
		...
		$scope.enableDonutDrawing = function () {
			actions.enableDonutDrawing();
			olmap.un('pointermove', mapOnFeatureSelect);
		...
		$rootScope.$on('web-mapping-select', mapOnFeatureSelect);
		$rootScope.$on('web-mapping-deselect', mapOnFeatureDeselect);
